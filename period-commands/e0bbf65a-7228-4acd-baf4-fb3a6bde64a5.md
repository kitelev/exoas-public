...
exo__Asset_updatedAt: 2026-09-06T22:39:32
exo__Instance_class:
...
Dynamic command that sets `ems__Effort_startTimestamp` — the ACTUAL start — to a user-provided datetime. For digitizing an effort **post-factum** (backdate the real start when the task is logged later than it actually began). Inline button on `ems__Task` and `ems__Project` (bindings added 2026-09-06, ticket 61a72179); also invocable as the CLI verb `set-start-timestamp`. Mirrors `Set Planned Start` (6bc86da6), targeting the fact timestamp.
